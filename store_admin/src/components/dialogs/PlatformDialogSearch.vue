...
function handleQuery(query?: string) {
  emit('change:query', query == '' ? undefined : query)
}
    ...
    <div class="relative h-full flex flex-col" @keyup="handleKeyup">
      <div class="gap-3 mb-3 mt-3">
        <div class="relative w-full">
          ...
          <InputText autofocus v-model="query" @update:model-value="handleQuery" placeholder="Search" size="large" class="pr-10 w-full" />
        </div>
      </div>
