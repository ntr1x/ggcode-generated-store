...
export type GridPromotionsRecord = Record<string, any>

export type GridPromotionsPage = {
  content: GridPromotionsRecord[]
}

export type GridPromotionsProps = {
  state: State<GridPromotionsPage>,
  scrollable?: boolean,
  scrollHeight?: string,
  hideId?: boolean,
  hideName?: boolean,
...
const selection = defineModel<GridPromotionsRecord[]>('selection')

withDefaults(defineProps<GridPromotionsProps>(), {
  scrollable: true,
  scrollHeight: 'flex',
})
...
<template>
  ...
  >
    ...
    <Column v-if="!hideId" header="Id">
      <template #body="slotProps">
        <SymbolId :value="slotProps.data.id" :label="'REF'" :route="`/admin/promotions/${slotProps.data.id}`"/>
      </template>
    </Column>
    <Column v-if="!hideName" field="name" header="Name" />
    <Column v-if="!hidePromotionType" header="Promotion Type">
      <template #body="slotProps">
        <SymbolPromotionType :value="slotProps.data.type"/>
      </template>
    </Column>
  </DataTable>
</template>
