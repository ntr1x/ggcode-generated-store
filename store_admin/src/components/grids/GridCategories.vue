...
export type GridCategoriesRecord = Record<string, any>

export type GridCategoriesPage = {
  content: GridCategoriesRecord[]
}

export type GridCategoriesProps = {
  state: State<GridCategoriesPage>,
  scrollable?: boolean,
  scrollHeight?: string,
  hideId?: boolean,
  hideName?: boolean,
...
const selection = defineModel<GridCategoriesRecord[]>('selection')

withDefaults(defineProps<GridCategoriesProps>(), {
  scrollable: true,
  scrollHeight: 'flex',
})
...
<template>
  ...
  >
    ...
    <Column v-if="!hideId" header="Id">
      <template #body="slotProps">
        <SymbolId :value="slotProps.data.id" :label="'REF'" :route="`/admin/categories/${slotProps.data.id}`"/>
      </template>
    </Column>
