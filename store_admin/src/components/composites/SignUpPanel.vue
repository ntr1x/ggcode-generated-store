...
<template>
  <div class="grid grid-cols-1 w-full md:grid-cols-2 md:w-[40rem] overflow-hidden shadow-sm bg-white">
    <div class="p-6 md:p-9 flex text-white dark:text-surface-900 bg-blue-500 dark:bg-blue-400">
      <div class="m-auto grid grid-cols-1 gap-4 text-center">
        ...
        <div>
          ...
          <RouterLink to="/sign-in" custom v-slot="{ navigate }">
            <Button @click="navigate" label="Sign In" text class="w-full text-white" rounded severity="info" />
          </RouterLink>
        </div>
      </div>
    </div>
