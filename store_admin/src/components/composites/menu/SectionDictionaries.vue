...
<template>
  <div class="mb-3" v-if='hasAnyRole(["realm:developer","realm:admin"])'>
    ...
    <ul class="list-none p-0 m-0 overflow-hidden">
      ...
      <li v-if='hasAnyRole(["realm:developer","realm:admin"])'>
        <router-link to="/admin/template_types" class="flex items-center cursor-pointer p-1 rounded-md text-surface-700 dark:text-surface-0/80 hover:bg-surface-100 dark:hover:bg-surface-700 duration-200 transition-colors">
          <i class="pi pi-database mr-2"></i>
          <span class="font-medium">Template Types</span>
        </router-link>
      </li>
      <li v-if='hasAnyRole(["realm:developer","realm:admin"])'>
        <router-link to="/admin/template_shapes" class="flex items-center cursor-pointer p-1 rounded-md text-surface-700 dark:text-surface-0/80 hover:bg-surface-100 dark:hover:bg-surface-700 duration-200 transition-colors">
          <i class="pi pi-database mr-2"></i>
          <span class="font-medium">Template Shapes</span>
        </router-link>
      </li>
      <li v-if='hasAnyRole(["realm:developer","realm:admin"])'>
        <router-link to="/admin/network_types" class="flex items-center cursor-pointer p-1 rounded-md text-surface-700 dark:text-surface-0/80 hover:bg-surface-100 dark:hover:bg-surface-700 duration-200 transition-colors">
          <i class="pi pi-database mr-2"></i>
          <span class="font-medium">Network Types</span>
        </router-link>
      </li>
    </ul>
  </div>
