...
const label = computed(() => {
  return props.label != undefined
    ? props.label
    : props.value == null ? null : [props.value.name, props.value.patronymic, props.value.surname].filter(item => item != null).join(" ")
})

const route = computed(() => {
  return props.route != undefined
    ? props.route
    : props.value == null ? null : `/admin/customers/${props.value.id}`
})
