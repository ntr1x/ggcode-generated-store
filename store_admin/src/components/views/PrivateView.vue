...
<template>
  <PrincipalProvider>
    <SocketConnection name="assembly_web:ws" uri="ws://api.local.example.com/api/assembly_web/ws" />
    <SocketSubscription socket-name="assembly_web:ws" :selectors="[
      {
        headers: {
          channel: { type: 'EQUAL', value: 'public_dispatch' }
        }
      }
    ]" />
