WITH
  ...
  selection_assets (id, product_id, color) AS (
    SELECT
  ...
  selection_images (id, product_id, type_id, original_id, uri, name) AS (
    SELECT
      CASE
    ...
    FROM "public"."product" p
    INNER JOIN selection_assets a ON a.product_id = p.id
    CROSS JOIN selection_formats f
  )
INSERT INTO "public"."product_image" (id, product_id, type_id, original_id, uri, name)
SELECT id, product_id, type_id, original_id, uri, name
