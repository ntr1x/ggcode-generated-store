...
import com.example.service.payments.model.PublicPaymentModel;
...
public interface PublicPaymentEntityConverter {

    @RequiredArgsConstructor
    @Component("servicePayments.publicPaymentEntityConverter.toModel")
    class ToModel implements AppConverter<PublicPaymentEntity, PublicPaymentModel> {
        private final ApplicationContext applicationContext;

        public PublicPaymentModel convert(PublicPaymentEntity source) {
            ConversionService conversionService = applicationContext.getBean(ConversionService.class);

            return PublicPaymentModel.builder()
                    .id(source.getId())
