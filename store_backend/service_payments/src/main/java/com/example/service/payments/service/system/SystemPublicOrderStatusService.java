...
public class SystemPublicOrderStatusService {
    ...
    public SystemPublicOrderStatusResponse.RemoveAll removeAll(
        SystemPublicOrderStatusRequest.Context context,
        Collection<SystemPublicOrderStatusRequest.Id> keys
    ) {
        Collection<PublicOrderStatusEntity> entities = publicOrderStatusRepository
                .findAllById(keys.stream().map(SystemPublicOrderStatusRequest.Id::getId).toList());

        Collection<SystemPublicOrderStatusModel> removed = entities
                .stream()
                .map(entity -> conversionService.convert(entity, SystemPublicOrderStatusModel.class))
                .toList();

        publicOrderStatusRepository.deleteAll(entities);

        return SystemPublicOrderStatusResponse.RemoveAll
            .builder()
            .removed(removed)
            .build();
    }
