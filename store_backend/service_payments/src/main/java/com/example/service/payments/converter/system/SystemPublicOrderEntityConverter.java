...
public interface SystemPublicOrderEntityConverter {

    @RequiredArgsConstructor
    @Component("servicePayments.systemPublicOrderEntityConverter.toModel")
    class ToModel implements AppConverter<PublicOrderEntity, SystemPublicOrderModel> {
        private final ApplicationContext applicationContext;

        public SystemPublicOrderModel convert(PublicOrderEntity source) {
            ConversionService conversionService = applicationContext.getBean(ConversionService.class);

            return SystemPublicOrderModel.builder()
                    .id(source.getId())
