...
public class SystemPublicPromotionService {
    ...
    public SystemPublicPromotionResponse.RemoveAll removeAll(
        SystemPublicPromotionRequest.Context context,
        Collection<SystemPublicPromotionRequest.Id> keys
    ) {
        Collection<PublicPromotionEntity> entities = publicPromotionRepository
                .findAllById(keys.stream().map(SystemPublicPromotionRequest.Id::getId).toList());

        Collection<SystemPublicPromotionModel> removed = entities
                .stream()
                .map(entity -> conversionService.convert(entity, SystemPublicPromotionModel.class))
                .toList();

        publicPromotionRepository.deleteAll(entities);

        return SystemPublicPromotionResponse.RemoveAll
            .builder()
            .removed(removed)
            .build();
    }
