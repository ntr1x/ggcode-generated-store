...
public class SystemPublicPromotionTypeService {
    ...
    @Transactional
    public SystemPublicPromotionTypeResponse.RemoveAll removeAll(
        SystemPublicPromotionTypeRequest.Context context,
        Collection<SystemPublicPromotionTypeRequest.Id> keys
    ) {
        Collection<PublicPromotionTypeEntity> entities = publicPromotionTypeRepository
                .findAllById(keys.stream().map(SystemPublicPromotionTypeRequest.Id::getId).toList());

        Collection<SystemPublicPromotionTypeModel> removed = entities
                .stream()
                .map(entity -> conversionService.convert(entity, SystemPublicPromotionTypeModel.class))
                .toList();

        publicPromotionTypeRepository.deleteAll(entities);

        return SystemPublicPromotionTypeResponse.RemoveAll
            .builder()
            .removed(removed)
            .build();
    }
